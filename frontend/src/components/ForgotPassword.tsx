...
const ForgotPassword: React.FC<ForgotPasswordProps> = ({ onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:8000'}/password/forgot`, {
        method: 'POST',
        headers: {
      ...
      <div className="min-h-screen bg-gradient-to-b from-purple-50 via-white to-blue-50 flex items-center justify-center px-4 py-12">
        <div className="max-w-md w-full">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
            ...
            <button
              onClick={onBack}
              className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-bold py-3 px-6 rounded-lg transition-all duration-200 transform hover:scale-[1.02] shadow-lg hover:shadow-xl"
            >
              Volver al Login
            </button>
          </div>
